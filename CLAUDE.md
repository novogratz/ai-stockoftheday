...
- `universe.py` — `load_universe()` fetches the full US common-stock listing (~6,000 symbols, pennies included) from the NASDAQ Trader symbol directory, filtered (no ETFs/warrants/preferreds/test issues) and cached 7 days in `data/us_universe.json`; falls back to `WATCHLIST` (~450 hardcoded liquid names, also reachable via `--core`). `SECTOR_MAP` (symbol → sector ETF) stays manual and only covers the core names.
...
- `portfolio.py` — paper-trading ledger (`portfolio/ledger.json`, gitignored but **persistent user data — never delete it when clearing caches**): one row per (date, symbol), first pick of each day is the ⭐ portfolio pick. A $10k simulated portfolio goes all-in on that pick and sells at close, compounding daily via `settle()` (lazy — safe to call anytime, only prices sessions that have ended, retries unpriceable rows). All I/O boundaries (`fetch_close`, `live_fetch`, `when`, `ledger`) are injectable for offline tests.
- `cli.py` — subcommands plus `run`, a 60s-tick loop that sends a startup report immediately, one morning report per weekday, an intraday report every 30 min during the session, and an end-of-day recap after 16:05 ET. Reports record their picks into the ledger and append portfolio status. `MarketData` is reused for the whole calendar day (daily bars are static intraday; only the live-gap fetch in `strategy.scan()` refreshes) — don't "fix" it back to per-cycle instances, full-universe downloads are expensive.
